...
COMPARE_WRITE_TOHOST_ADDR = $(shell if [ -n "$(COMPARE_ELF)" ] && [ -f "$(COMPARE_ELF)" ]; then $(NM) -n "$(COMPARE_ELF)" 2>/dev/null | awk '$$3 == "write_tohost" { print $$1; exit }'; fi)
COMPARE_SIM_DEFINES_WITH_TOHOST = $(strip $(COMPARE_SIM_EXTRA_DEFINES) $(if $(COMPARE_TOHOST_ADDR),+tohost_addr=$(COMPARE_TOHOST_ADDR)) $(if $(COMPARE_WRITE_TOHOST_ADDR),+write_tohost_addr=$(COMPARE_WRITE_TOHOST_ADDR)))
