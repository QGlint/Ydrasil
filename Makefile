...
PPA_DIR ?= $(BUILD_DIR)/PPA$(FPU_PROFILE_SUFFIX)
...
COE_LOOP5_EXPECT_CNT ?= $(if $(filter 1,$(FPU)),0x00000002,0x00000001)
COE_LOOP5_EXPECT_SEG ?= $(if $(filter 1,$(FPU)),0x37800002,0x37800001)
...
export PROJECT_ROOT BUILD_DIR WAVE_DIR LOG_DIR SIM_TOOL IP VERILATOR_MOD COVERAGE VERILATOR_COVERAGE UVM USE_BENDER BENDER DIV_IMPL FPU FPU_PRECISION LSU_IMPL MEMS_IMPL ARCH ABI RISCV_PREFIX CC OBJCOPY OBJDUMP GDB QEMU TRACE_TO_CSV TRACE_COMPARE
...
SYN_RTL_DEFINES += $(if $(and $(filter 1,$(FPU)),$(filter double,$(FPU_PRECISION))),YDRASIL_FPU_DOUBLE)
SYN_PROFILE ?= official
SYN_PROFILE_SUFFIX = $(if $(filter official,$(SYN_PROFILE)),,-$(SYN_PROFILE))$(if $(filter 1,$(FPU)),-fpu-$(FPU_PRECISION))
SYN_ENABLE_ILA ?= 0
SYN_BOARD_XDC ?=
...
coe_loop5: coe_loop5_gen
	@$(MAKE) coe_simple \
		...
		COE_EXPECT_CNT_READ=$(COE_LOOP5_EXPECT_CNT) \
		COE_EXPECT_SEG=$(COE_LOOP5_EXPECT_SEG)
