...
module ydrmem_ram #(
    ...
    always @(posedge clk) begin
        if (wen_i) begin
            // 根据掩码对每个字节单独处理
            if (we_mask_i[0]) mem_r[waddr_i][7:0] <= data_i[7:0];
            if (we_mask_i[1]) mem_r[waddr_i][15:8] <= data_i[15:8];
            if (we_mask_i[2]) mem_r[waddr_i][23:16] <= data_i[23:16];
            if (we_mask_i[3]) mem_r[waddr_i][31:24] <= data_i[31:24];
        end
    end
