...
(
    ...
    input  wire [REGS_DATA_WIDTH-1:0]   pipe1_alu_wdata_rd_i,
	    ...
	    input  wire                         pipe1_alu_rob_valid_i,
	    input  wire [5:0]                   pipe1_alu_rob_idx_i,
    ...
    output wire                         pipe1_wb_enqueue_o,
	    ...
	    output wire                         pipe1_wb_rob_valid_o,
	    output wire [5:0]                   pipe1_wb_rob_idx_o,
...
`endif
	    ...
	    reg p1_fifo_rob_valid_q [0:P1_FIFO_DEPTH-1];
	    reg [5:0] p1_fifo_rob_idx_q [0:P1_FIFO_DEPTH-1];
    ...
    wire [REGS_DATA_WIDTH-1:0] p1_fifo_head_data = p1_fifo_data_q[p1_fifo_rptr_q];
	    ...
	    wire p1_fifo_head_rob_valid = p1_fifo_rob_valid_q[p1_fifo_rptr_q];
	    wire [5:0] p1_fifo_head_rob_idx = p1_fifo_rob_idx_q[p1_fifo_rptr_q];
...
`endif
    ...
    assign pipe1_wb_enqueue_o = p1_enqueue_accept | p1_direct_write;
	    ...
	    assign pipe1_wb_rob_valid_o =
	        p1_fifo_ready_event ? p1_fifo_head_rob_valid :
	        (p1_current_ready_event & pipe1_alu_rob_valid_i);
	    assign pipe1_wb_rob_idx_o =
	        p1_fifo_ready_event ? p1_fifo_head_rob_idx : pipe1_alu_rob_idx_i;
        ...
        if (!rst_n) begin
            ...
            p1_fifo_count_q     <= '0;
	            for (p1_i = 0; p1_i < P1_FIFO_DEPTH; p1_i = p1_i + 1) begin
	                ...
	                p1_fifo_rob_valid_q[p1_i] <= 1'b0;
	                p1_fifo_rob_idx_q[p1_i] <= '0;
	            end
...
`endif
            ...
            if (p1_enqueue_accept) begin
	                ...
	                p1_fifo_rob_valid_q[p1_fifo_wptr_q] <= pipe1_alu_rob_valid_i;
	                p1_fifo_rob_idx_q[p1_fifo_wptr_q] <= pipe1_alu_rob_idx_i;
