...
import ydrasil_pkg::*;
 #(
	...
	input  ydrasil_issue_state_pkt_t         issue_state_i,
	...
	ydrasil_lsu_req_pkt_t                 lsu_req_o;
	ydrasil_fpu_req_pkt_t                 fpu_req_o;
    ...
    reg                                  id_ex_jalr_ff;
	ydrasil_bypass_sel_t                  bypass_rs1_q;
	ydrasil_bypass_sel_t                  bypass_rs2_q;
	...
	ydrasil_bypass_sel_t                  dual_bypass_rs1_q;
	ydrasil_bypass_sel_t                  dual_bypass_rs2_q;
    ...
    assign hzd_status_i = issue_state_i.slot0.hazard;
    assign hzd_status1_i = issue_state_i.slot1.hazard;
    assign wb_fwd_i = issue_state_i.wb.wb0;
    assign wb_fwd1_i = issue_state_i.wb.wb1;
    assign producer_rs1_fwd_i = issue_state_i.slot0.src1;
    assign producer_rs2_fwd_i = issue_state_i.slot0.src2;
    assign producer_rs3_fwd_i = issue_state_i.slot1.src1;
    assign producer_rs4_fwd_i = issue_state_i.slot1.src2;
...
`endif
	...
	// Variable-latency completions terminate at the registered Future File.
	// Issue has no completion-bus compare/priority/data path. A load is treated
	// as fixed-latency only when its base needs no EX bypass and its address is
	// already known to target DTCM.
	wire [BUS_ADDR_WIDTH-1:0] issue0_pre_agu_addr =
		issue_rs1_data + decode_pkt_i.imm;
	wire [BUS_ADDR_WIDTH-1:0] issue1_pre_agu_addr =
		issue1_rs1_data + decode_pkt1_i.imm;
	always_comb begin
		issue_feedback_o = '0;
		issue_feedback_o.slot0_fixed_dtcm_load = issue_pkt_i.valid &&
			decode_pkt_i.operator_type[OPERATOR_TYPE_LOAD] &&
			(issue_state_i.slot0.bypass_rs1 == BYPASS_NONE) &&
			(issue0_pre_agu_addr[31:DTCM_ADDR_WIDTH+2] ==
			 DTCM_BASE_ADDR[31:DTCM_ADDR_WIDTH+2]);
		issue_feedback_o.slot1_fixed_dtcm_load = issue_pkt1_i.valid &&
			decode_pkt1_i.operator_type[OPERATOR_TYPE_LOAD] &&
			(issue_state_i.slot1.bypass_rs1 == BYPASS_NONE) &&
			(issue1_pre_agu_addr[31:DTCM_ADDR_WIDTH+2] ==
			 DTCM_BASE_ADDR[31:DTCM_ADDR_WIDTH+2]);
	end
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            id_instr_addr_ff <= '0;
            id_ex_jalr_ff <= 1'b0;
				bypass_rs1_q <= BYPASS_NONE;
			bypass_rs2_q <= BYPASS_NONE;
			...
			dual_bypass_rs1_q <= BYPASS_NONE;
			dual_bypass_rs2_q <= BYPASS_NONE;
            dual_rf_waddr_q <= '0;
            ...
            if (!stall_id_i) begin
                ...
                producer_id_ff      <= issue_state_i.slot0.producer_id;
                producer_tracked_ff <= issue_state_i.slot0.producer_tracked;
				...
				bypass_rs1_q <= issue_valid_ff ?
					issue_state_i.slot0.bypass_rs1 : BYPASS_NONE;
				bypass_rs2_q <= issue_valid_ff ?
					issue_state_i.slot0.bypass_rs2 : BYPASS_NONE;
				...
				dual_bypass_rs1_q <= pair_issue ?
					issue_state_i.slot1.bypass_rs1 : BYPASS_NONE;
				dual_bypass_rs2_q <= pair_issue ?
					issue_state_i.slot1.bypass_rs2 : BYPASS_NONE;
                dual_rf_waddr_q <= decode_pkt1_i.rd_addr;
                dual_producer_id_q <= issue_state_i.slot1.producer_id;
                dual_producer_tracked_q <=
                    issue_state_i.slot1.producer_tracked;
	...
	always_comb begin
		issue_ex_o = '0;
		issue_ex_o.operand_a = operand_a_ff;
		issue_ex_o.operand_b = operand_b_ff;
		issue_ex_o.operator_info = operator_ff;
		issue_ex_o.operator_type = operator_type_ff;
		issue_ex_o.valid = id_ex_valid_ff;
		issue_ex_o.jalr = id_ex_jalr_ff;
			issue_ex_o.bypass_rs1 = bypass_rs1_q;
			issue_ex_o.bypass_rs2 = bypass_rs2_q;
		issue_ex_o.branch_target = id_ex_branch_pc_target_ff;
		issue_ex_o.branch_next_pc = id_ex_branch_next_pc_ff;
		issue_ex_o.branch_eq = 1'b0;
		issue_ex_o.branch_ge_signed = 1'b0;
		issue_ex_o.branch_ge_unsigned = 1'b0;
		issue_ex_o.bt_a_operand = bt_a_operand_ff;
		issue_ex_o.bt_b_operand = bt_b_operand_ff;
		issue_ex_o.pred_hit = id_ex_pred_hit_ff;
		issue_ex_o.pred_taken = id_ex_pred_taken_ff;
		issue_ex_o.pred_target = id_ex_pred_target_ff;
		issue_ex_o.pred_counter = id_ex_pred_counter_ff;
		issue_ex_o.pred_bht_index = id_ex_pred_bht_index_ff;
		issue_ex_o.csr_raddr = csr_reg_raddr_ff;
		issue_ex_o.csr_waddr = csr_ex_waddr_ff;
		issue_ex_o.csr_op_info = csr_op_info_ff;
		issue_ex_o.sys_op_info = sys_op_info_ff;
		issue_ex_o.pc = id_instr_addr_ff;
		issue_ex_o.fence_i = id_fence_i_ff;
		issue_ex_o.rd_wen = rf_wen_rd_ff;
		issue_ex_o.rd_addr = rf_waddr_rd_ff;
		issue_ex_o.producer_id = producer_id_ff;
		issue_ex_o.producer_tracked = producer_tracked_ff;
		issue_ex_o.lsu_req = lsu_req_o;
		issue_ex_o.fpu_req = fpu_req_o;
		issue_ex_o.lane1_operand_a = dual_operand_a_q;
		issue_ex_o.lane1_operand_b = dual_operand_b_q;
		issue_ex_o.lane1_operator_info = dual_operator_q;
		issue_ex_o.lane1_operator_type = dual_operator_type_q;
		issue_ex_o.lane1_operator_lsu = dual_operator_lsu_q;
		issue_ex_o.lane1_store_data = dual_store_data_q;
			issue_ex_o.lane1_store_data_valid = dual_store_data_valid_q;
			issue_ex_o.lane1_bypass_rs1 = dual_bypass_rs1_q;
			issue_ex_o.lane1_bypass_rs2 = dual_bypass_rs2_q;
		issue_ex_o.lane1_rd_addr = dual_rf_waddr_q;
		issue_ex_o.lane1_producer_id = dual_producer_id_q;
		issue_ex_o.lane1_producer_tracked = dual_producer_tracked_q;
		issue_ex_o.lane1_valid = dual_valid_q;
		issue_ex_o.lane1_pc = dual_pc_q;
		issue_ex_o.lane1_instr = dual_instr_q;
	end
...
(
    ...
    input  ydrasil_bypass_sel_t           bypass_rs1_i,
    input  ydrasil_bypass_sel_t           bypass_rs2_i,
    input  wire                           lane0_bypass_valid_i,
    input  wire [REGS_DATA_WIDTH-1:0]     lane0_bypass_data_i,
    ...
    output wire                           lane1_bypass_valid_o,
    output wire [REGS_DATA_WIDTH-1:0]     lane1_bypass_data_o,
    ...
    reg memory_q;
    reg load_q;
    wire use_lane0_rs1 = (bypass_rs1_i == BYPASS_LANE0) &&
        lane0_bypass_valid_i;
    wire use_lane0_rs2 = (bypass_rs2_i == BYPASS_LANE0) &&
        lane0_bypass_valid_i;
    wire use_lane1_rs1 = (bypass_rs1_i == BYPASS_LANE1) && valid_q &&
        !memory_q;
    wire use_lane1_rs2 = (bypass_rs2_i == BYPASS_LANE1) && valid_q &&
        !memory_q;
    wire [REGS_DATA_WIDTH-1:0] exec_operand_a = use_lane0_rs1 ?
        lane0_bypass_data_i : use_lane1_rs1 ? result_q : operand_a_i;
    wire [REGS_DATA_WIDTH-1:0] exec_operand_b = use_lane0_rs2 ?
        lane0_bypass_data_i : use_lane1_rs2 ? result_q : operand_b_i;
    wire [REGS_DATA_WIDTH-1:0] alu_result;
    wire [REGS_DATA_WIDTH-1:0] full_bitmanip_result;
    ...
    ydrasil_bitmanip u_dual_bitmanip (
        .operand_a_i(exec_operand_a),
        .operand_b_i(exec_operand_b),
        .operator_i(operator_i),
        .operator_type_i(operator_type_i),
        .result_o(full_bitmanip_result)
    );
    ...
    always_comb begin
        ...
        lsu_req_o.store_data = (use_lane0_rs2 || use_lane1_rs2) ?
            exec_operand_b : store_data_i;
    ...
    assign completion_o.data = operator_type_i[OPERATOR_TYPE_BITMANIP] ?
        full_bitmanip_result : alu_result;
    assign lane1_bypass_valid_o = valid_q && !memory_q && (rd_addr_q != '0);
    assign lane1_bypass_data_o = result_q;
