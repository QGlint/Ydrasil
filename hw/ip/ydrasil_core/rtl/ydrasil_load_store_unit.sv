...
(
    ...
    input wire                            ex_lsu_store_data_fwd_valid_i,
    input wire [ydrasil_pkg::REGS_DATA_WIDTH-1:0]      ex_lsu_store_data_fwd_i,
    ...
    if (LSU_MODE == LSU_MODE_NEW) begin : g_new
        ...
        assign lsu_rs2_data =
            ex_lsu_store_data_fwd_valid_i ? ex_lsu_store_data_fwd_i :
                                             id_lsu_rs2_data_i;
    ...
    localparam int LEGACY_DTCM_TAG_LSB = ydrasil_pkg::DTCM_ADDR_WIDTH + 2;

    wire is_load;
    wire is_store;
    wire request_valid;
    wire [ydrasil_pkg::BUS_ADDR_WIDTH-1:0] mem_addr;
    wire [1:0] mem_addr_index;
    ...
    wire [31:0] store_rs2_data;
    wire store_is_sb;
    wire store_is_sh;
    wire store_is_sw;
    wire [3:0] store_sb_mask;
    wire [3:0] store_sh_mask;
    wire [31:0] store_sb_data;
    wire [31:0] store_sh_data;
    ...
    assign request_is_dtcm =
        mem_addr[ydrasil_pkg::BUS_ADDR_WIDTH-1:LEGACY_DTCM_TAG_LSB] ==
        ydrasil_pkg::DTCM_BASE_ADDR[ydrasil_pkg::BUS_ADDR_WIDTH-1:LEGACY_DTCM_TAG_LSB];
    ...
    assign store_rs2_data =
        ex_lsu_store_data_fwd_valid_i ? ex_lsu_store_data_fwd_i :
                                        id_lsu_rs2_data_i;
    assign store_is_sb = operator_lsu_i[ydrasil_pkg::OP_LSU_SB];
    assign store_is_sh = operator_lsu_i[ydrasil_pkg::OP_LSU_SH];
    assign store_is_sw = operator_lsu_i[ydrasil_pkg::OP_LSU_SW];
    assign store_sb_mask =
        ({4{mem_addr_index == 2'b00}} & 4'b0001) |
        ({4{mem_addr_index == 2'b01}} & 4'b0010) |
        ({4{mem_addr_index == 2'b10}} & 4'b0100) |
        ({4{mem_addr_index == 2'b11}} & 4'b1000);
    assign store_sh_mask = mem_addr_index[1] ? 4'b1100 : 4'b0011;
    assign store_wmask =
        ({4{store_is_sb}} & store_sb_mask) |
        ({4{store_is_sh}} & store_sh_mask) |
        ({4{store_is_sw}} & 4'b1111);
    assign store_sb_data =
        ({32{mem_addr_index == 2'b00}} & {24'b0, store_rs2_data[7:0]}) |
        ({32{mem_addr_index == 2'b01}} & {16'b0, store_rs2_data[7:0], 8'b0}) |
        ({32{mem_addr_index == 2'b10}} & {8'b0, store_rs2_data[7:0], 16'b0}) |
        ({32{mem_addr_index == 2'b11}} & {store_rs2_data[7:0], 24'b0});
    assign store_sh_data =
        mem_addr_index[1] ? {store_rs2_data[15:0], 16'b0} :
                            {16'b0, store_rs2_data[15:0]};
    assign store_wdata =
        ({32{store_is_sb}} & store_sb_data) |
        ({32{store_is_sh}} & store_sh_data) |
        ({32{store_is_sw}} & store_rs2_data);
